...
load_file <- function(file){
  dt <- fread(file)
  dt <- dt[time_id==0, list(p_val = mean(p)), by=rep_id] #could add time_id==0 in case all time steps are output
...
list_of_files <- list.files(dir, pattern="pred_pre", full.names = T)
length(list_of_files)==sum(params$B)
files_all_pre <- rbindlist(lapply(list_of_files, load_file))
