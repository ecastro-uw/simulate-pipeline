# Model 25: LME with ARMA(1,1) errors
...
model_25 <- function(dataset, w, d, use_param_uncertainty = TRUE) {
  ...
  # Build forecast data: lagged_edu at T+1 = pct_edu at T
  last_time_step <- max(dt$time_id)
  new_dt <- dt[time_id == last_time_step, .(location_id, time_id, pct_edu)]
  new_dt[, time_id := last_time_step + w]
  setnames(new_dt, "pct_edu", "lagged_edu")

  # Design matrix
  X_new <- model.matrix(~lagged_edu, data = new_dt)  # n_loc x 2
