# Model 4: Linear Auto-Regressive Model 
...
model_4 <- function(dataset, w, d){
  ...
  # For the forecast row, cases_lag2_sum = cases_pc at t + cases_pc at t-1
  new_dt <- last_two[, .(
    time_id        = last_time_step + 1,
    lagged_y       = y[time_id == last_time_step],
    cases_lag2_sum = sum(cases_pc)   # sums t and t-1
  ), by = location_id]
  
  # Construct a matrix with new data values
  X_new <- model.matrix(~lagged_y + cases_lag2_sum, data=new_dt)
  ...
  ids <- data.table(model="model_4", location_id = new_dt$location_id, time_id = new_dt$time_id, sigma = sigma_hat)
