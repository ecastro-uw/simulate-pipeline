...
lm_cases <- function(dataset, w, d){
  ...
  # Lag cases to use as a predictor
  dt[, lagged_cases := shift(cases_pc), by=location_id]
  
  # Fit the model
  fit <- lm(y ~ lagged_y + lagged_cases, data = dt)
  ...
  new_dt <- dt[time_id==last_time_step, .(location_id, time_id, y, cases_pc)]
  new_dt$time_id <- last_time_step + 1
  setnames(new_dt, c('y', 'cases_pc'), c('lagged_y', 'lagged_cases'))
  
  # Construct a matrix with new data values
  X_new <- model.matrix(~lagged_y + lagged_cases, data=new_dt)
  ...
  ids <- data.table(model="lm_cases", location_id = new_dt$location_id, time_id = new_dt$time_id, sigma = sigma_hat)
