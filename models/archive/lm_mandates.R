lm_mandates <- function(dataset, w, d){
  ...
  # Lag the mandate to use as a predictor
  dt[, lagged_x := shift(pct_edu, n=1), by=location_id]
  dt[, lagged2_x := shift(pct_edu, n=2), by=location_id]
  
  # Fit the model
  fit <- lm(y ~ lagged_y + lagged_x + lagged2_x, data = dt)
  ...
  new_dt <- dt[time_id==last_time_step, .(location_id, time_id, lagged_y=y, lagged_x=pct_edu, lagged2_x = lagged_x)]
  new_dt$time_id <- last_time_step + 1
  
  # Construct a matrix with new data values
  X_new <- model.matrix(~lagged_y + lagged_x + lagged2_x, data=new_dt)
  ...
  ids <- data.table(model="lm_mandates", location_id = new_dt$location_id, time_id = new_dt$time_id, sigma = sigma_hat)
