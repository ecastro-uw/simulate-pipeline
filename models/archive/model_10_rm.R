# Model 10: Linear Auto-Regressive Model 
# Covariates: Gathering restrictions, Bar closures
# Note: Currently only supports w=1 (hard coded)

model_10 <- function(dataset, w, d){
  ...
  # Fit the model
  fit <- lm(y ~ lagged_y + lagged_gathering + lagged_bar, data = dt)
  ...
  new_dt <- dt[time_id==last_time_step, .(location_id, time_id, y, pct_gathering, pct_bar)]
  new_dt$time_id <- last_time_step + 1
  setnames(new_dt, c('y', 'pct_gathering', 'pct_bar'), c('lagged_y', 'lagged_gathering', 'lagged_bar'))
  
  # Construct a matrix with new data values
  X_new <- model.matrix(~lagged_y + lagged_gathering + lagged_bar, data=new_dt)
  ...
  ids <- data.table(model="model_10", location_id = new_dt$location_id, time_id = new_dt$time_id, sigma = sigma_hat)
