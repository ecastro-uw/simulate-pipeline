# Model 17: GLS with ARMA(1,1) errors
...
model_17 <- function(dataset, w, d, use_param_uncertainty = TRUE) {
  ...
  dt_complete <- dt[!is.na(lagged_gathering) & !is.na(y)]

  # Fit gls with ARMA(1,1) errors, pooled across all locations
  fit <- gls(y ~ lagged_gathering,
             data        = dt_complete,
