simple_lm <- function(dataset, w, d){
  ...
  new_dt <- dt[time_id==last_time_step, .(location_id, time_id, x)]
  new_dt$time_id <- last_time_step + 1
  setnames(new_dt, 'x', 'lagged_x')
  
  # Construct a matrix with new data values
  X_new <- model.matrix(~lagged_x, data=new_dt)
  ...
  ids <- data.table(model="simple_lm", location_id = new_dt$location_id, time_id = new_dt$time_id, sigma = sigma_hat)
