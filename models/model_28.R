# Model 28: LME with ARMA(1,1) errors
...
model_28 <- function(dataset, w, d, use_param_uncertainty = TRUE) {
  ...
  # Build forecast data: lagged_bar at T+1 = pct_bar at T
  last_time_step <- max(dt$time_id)
  new_dt <- dt[time_id == last_time_step, .(location_id, time_id, pct_bar)]
  new_dt[, time_id := last_time_step + w]
  setnames(new_dt, "pct_bar", "lagged_bar")

  # Design matrix
  X_new <- model.matrix(~lagged_bar, data = new_dt)  # n_loc x 2
